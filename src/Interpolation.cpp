...
double Interpolation::spline(const std::vector<double>& x, const std::vector<double>& y, double tx) { 
    ...
    /**
    * b (Main Diagonal)：主角。 代表**「這一點自己」**的係數。
    * a (Lower Diagonal)：下角。 代表**「前一點對這一點的影響」**的係數。
    * c (Upper Diagonal)：上角。 代表**「下一點對這一點的影響」**的係數。
    * d (Right Hand Side)：右側向量。 代表**「已知條件」**。
    */
    ...
    for (size_t i = 1; i < n - 1; ++i) {
        ...
        // b 裝入**「左邊那段」**的長度。2.0倍的原因是因為樣條的二階導數需要考慮左右兩邊的影響。
		b[i] = 2.0 * (h[i - 1] + h[i]);

        // c 裝入**「我自己這點」**負責的兩邊總長度。
        c[i] = h[i];

		// d 裝的是「目標弧度」，也就是**「這一點的彎曲程度」**。
        // 6.0倍的原因是因為樣條的二階導數在公式中會被 1/6 的係數調整，
        // 所以這裡預先乘以 6.0 是為了後續計算的方便。
	...
	// M[n-1] 已經是 d[n-1] / b[n-1] 了，因為最後一行的 b 和 d 已經被修改過了
    m[n - 1] = d[n - 1] / b[n - 1];
    for (int i = n - 2; i >= 0; --i) {
